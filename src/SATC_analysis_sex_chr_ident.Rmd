...
names(idx_o) <- basename(filenames_obtectus)

```

Modify the filterScaffold function from SATC. 
For some reason, if I run the normal one, in line 55 the rank() function returns floats, not ints, and therefore no overlap with 1:M, which returns in normScarfs being only FALSE,
which causes an error. Resolving ties with "first" returns integers instead, solving the problem.
...
rFilt_c <- filterScaffold_new(dat=idx_c,minLength=1e5,M=5)
rFilt_o <- filterScaffold_new(dat=idx_o,minLength=1e5,M=5)
...
When I execute the plotDepth function call as-is it doesn't work but executing each line of the function individually does? 
The below code is basically exactly copied from the plotDepth function included in SATC.
The colors in the plot are the different samples.
```{r}
species <- "c_chinensis" # "a_obtectus" or "c_chinensis"

if (species == "a_obtectus"){
  print("OBTECTUS")
  dat <- rFilt_o #rFilt_c or rFilt_a or rFilt_m or rFilt_o
}else if (species == "c_chinensis"){
  dat <- rFilt_c #rFilt_c or rFilt_a or rFilt_m or rFilt_o
}
...
ylim <- c(0,mmax)
if (species == "a_obtectus"){
  png("/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC/a_obtectus_norm_scaffold_cov.png",width = 2500, height = 800, res=170, bg="transparent")
}else if (species == "c_chinensis"){
  png("/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC/c_chinensis_norm_scaffold_cov.png",width = 2500, height = 800, res=170, bg="transparent")
}
par(mar = c(7, 7, 2, 3))
...
if (species == "a_obtectus"){
  title("A. obtectus normalized coverage per contig for each sample")
}else if (species == "c_chinensis"){
  title("C. chinensis normalized coverage per contig for each sample")
}
dev.off()

## original SATC function that doesn't work:
# plotDepth(rFilt_c, normOnly = T) ##all scaffs
```


Plot normalized coverage (from Axel's SATC code), the bottom normalized plot is the same as above 
```{r}
if (F){
  species <- "a_obtectus" # "a_obtectus" or "c_chinensis"

  if (species == "a_obtectus"){
    print("OBTECTUS")
    rFilt <- rFilt_o #rFilt_c or rFilt_a or rFilt_m or rFilt_o
    cols_list <- c("royalblue", "magenta4","red3","seagreen", "royalblue", "magenta4","red3","seagreen")
  }else if (species == "c_chinensis"){
    print("CHINENSIS")
    rFilt <- rFilt_c #rFilt_c or rFilt_a or rFilt_m or rFilt_o
    cols_list <- c("royalblue", "magenta4","red3","seagreen")
  }
  ...
  ggplot()+
    ...
    scale_fill_manual("Sample",values=cols_list)+
...
#obtectus
#all
sex_o_all <- sexDetermine(dat=rFilt_o, K=2, model="gaussian")
# sub(paste0(".sort.MarkDup.RG.filter.bam.idxstats", "$"), "", names(idx_o))
cbind(names(idx_o),sex_o_all$sex)
#only EL ant not LE
rFilt_o_EL <- rFilt_o[c('Sample_VA-3193-EL1-F.sort.MarkDup.RG.filter.bam.idxstats','Sample_VA-3193-EL1-M.sort.MarkDup.RG.filter.bam.idxstats','Sample_VA-3193-EL4-F.sort.MarkDup.RG.filter.bam.idxstats','Sample_VA-3193-EL4-M.sort.MarkDup.RG.filter.bam.idxstats')]
idx_o_EL <- lapply(filenames_obtectus[1:4],read.table,as.is=T)
names(idx_o_EL) <- sapply(basename(filenames_obtectus[1:4]), function(x) gsub(".sort.MarkDup.RG.filter.bam.idxstats", "",x))
sex_o <- sexDetermine(dat=rFilt_o_EL, K=2, model="gaussian")
cbind(names(idx_o_EL),sex_o$sex)
```
...
if (T){
  ...
  sex_pca_dat$sample<-c("FR1", "FR2", "M1", "M3")
  sex_pca_dat$sex<-sex_c$sex
  head(sex_pca_dat)
  ylims <- c(-1.5,1.5)
  xlims <- c(-3,3)
  png("/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC/sexing_PCA_chinensis_all_samples.png", width=15, height=15, units="cm", res=250,bg="transparent")
  title_str <- "C. chinensis SATC"
}
...
if (T){
  ...
  sex_pca_dat$sample <- gsub("^Sample_VA-3193-|\\.sort\\.MarkDup\\.RG\\.filter\\.bam\\.idxstats$", "", names(idx_o_EL))

  sex_pca_dat$sex<-sex_o$sex
  ylims <- c(-2,2)
  xlims <- c(-3,3)
  png("/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC/sexing_PCA_obtectus_F0_EL.png", width=15, height=15, units="cm", res=250,bg="transparent")
  title_str <- "A. obtectus (only EL) SATC"
}

# obtectus all samples
if (T){
  mat <- sapply(sex_o_all$dat,function(y) y$norm) 
  sex_pca_dat<-as.data.frame(sex_o_all$pca$x)
  #sex_pca_dat$sample<-sub(paste0(".sort.MarkDup.RG.filter.bam.idxstats", "$"), "", names(idx_o))
  sex_pca_dat$sample <- gsub("^Sample_VA-3193-|\\.sort\\.MarkDup\\.RG\\.filter\\.bam\\.idxstats$", "", names(idx_o))
  sex_pca_dat$sex<-sex_o_all$sex
  ylims <- c(-2.2,2.2)
  xlims <- c(-2.5,3.5)
  png("/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC/sexing_PCA_obtectus_F0_all_samples.png", width=15, height=15, units="cm", res=250,bg="transparent")
  title_str <- "A. obtectus SATC"
}
...
if (T){ 
  ...
  plot_directory="/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC"
  plot_filename="contig_coverage_sex_ratio_chinensis.png"
  table_filename="contig_coverage_sex_ratio_chinensis.csv"
...
if (T){ 
  plot_title = "A. obtectus F0 individuals" 
  plot_directory="/Users/miltr339/work/PhD_code/PhD_chapter2/data/SATC"
  plot_filename="contig_coverage_sex_ratio_obtectus.png"
  table_filename="contig_coverage_sex_ratio_obtectus.csv"
  ...
  # get list of contigs with X and Y homology according to blast, can be highlighted separately in the plot
  #x_homologs <- as.character(read.csv("/Users/milena/work/sex_chromosome_homology/chinensis_maculatus_x_homology_contig_list.csv", header=FALSE))
  #y_homologs <- as.character(read.csv("/Users/milena/work/sex_chromosome_homology/chinensis_maculatus_y_homology_contig_list.csv", header=FALSE))
  #homologs <- c(x_homologs, y_homologs)
  homologs <- c()
...
if (TRUE){
  color_gradient <- colorRampPalette(c("#255CF5", "#909bb5", "#BD351E")) # blue,gray,red
  ...
  # add transparency
  point_colors <- add_alpha(point_colors, alpha=0.75)
...
if(T){
  ...
  png(filename = paste(plot_directory, plot_filename, sep="/"), width=17, height=13, units="cm", res=200, bg="transparent")
...
if (T){
  ...
  write.csv(SC_contigs, file = paste(plot_directory, table_filename, sep="/"))
  
  print(paste("contig_list saved to:", paste(plot_directory, table_filename, sep="/"), sep=" "))
}
...
Look at the scaffolds individually if there are too many, e.g. when the min scaffold length threshold is really low.
...
plot(sig_dif_scafs_dat$contig, 
     ...
     las = 2, # rotate the x axis ticks by 90° to make them all fit
