...
function parseNewick(str) {
  ...
  function renderTree(newickStr) {
    ...
    // after computing root/leafCount, before using marginRight:
    let maxLabelLength = 0;
    (function findMaxLabel(n) {
    if (!n.children.length) maxLabelLength = Math.max(maxLabelLength, n.name.length);
    n.children.forEach(findMaxLabel);
    })(root);

    const marginRight = Math.max(160, maxLabelLength * 8 + 40); // 40px padding buffer
  ...
  const textarea = document.getElementById('newick');
  renderTree(textarea.value);
